...
//
// Also builds lowercase.golden.json: every character whose JavaScript toLowerCase() is not itself,
// and what it becomes. Matching ignores case, so the C# port's lower-casing must agree.
//
//   node tests/golden/generate.js          rewrite both files
//   node tests/golden/generate.js --check  exit 1 if either file on disk is not what this would write
...
const OUT_LOWERCASE = path.join(HERE, "lowercase.golden.json");
...
add("salary: a hexadecimal salary string is read like JavaScript reads it", pay({ job_min_salary: "0x4E20", job_max_salary: "0b110000110101000" }), { skills: [], preferences: prefs30to60 });
add("salary: true counts as 1 and false as not given", pay({ job_min_salary: true, job_max_salary: false }), { skills: [], preferences: { minSalary: 30000 } });
add("salary: a negative maximum scores below zero", pay({ job_max_salary: -5000 }), { skills: [], preferences: { minSalary: 30000 } });
add("salary: an infinite maximum is open-ended", pay({ job_min_salary: 10, job_max_salary: "Infinity" }), { skills: [], preferences: { minSalary: 30000 } });
add("salary: a maximum too big for a number is open-ended", pay({ job_min_salary: 10, job_max_salary: "1e999" }), { skills: [], preferences: { minSalary: 30000 } });
...
function buildLowercase() {
    ...
    for (let cp = 0; cp <= 0x10FFFF; cp++) {
        ...
        if (lower !== original) map[cp.toString(16)] = Array.from(lower).map(ch => ch.codePointAt(0).toString(16)).join(" ");
    }

    return "{\n" +
        `"about": "Every character (as hex code points) whose JavaScript toLowerCase() is not itself, and what it becomes. Generated by tests/golden/generate.js - do not edit by hand.",\n` +
        `"count": ${Object.keys(map).length},\n` +
        '"map": {\n' + Object.entries(map).map(([cp, lower]) => `"${cp}": "${lower}"`).join(",\n") + "\n}\n}\n";
}
...
const lowercaseText = buildLowercase();

if (process.argv.includes("--check")) {
    const read = file => fs.existsSync(file) ? fs.readFileSync(file, "utf8").replace(/\r\n/g, "\n") : null;
    const current = read(OUT) === text && read(OUT_LOWERCASE) === lowercaseText;
    console.log(current ? `golden file is current (${withExpected.length} cases)` : "golden file is OUT OF DATE - run: node tests/golden/generate.js");
    process.exit(current ? 0 : 1);
}

fs.writeFileSync(OUT, text);
fs.writeFileSync(OUT_LOWERCASE, lowercaseText);
